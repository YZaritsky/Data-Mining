...
def extract_country_or_place(text):
    if pd.isna(text):
        return text

    # List of all countries using pycountry
    countries = [country.name for country in pycountry.countries]
    # Adding notable fictional places
    ...
    # Using regex to find common places within the text
...
def search_wiki(url, search_term):
    search_url = f"{url}{search_term.replace(' ', '_')}"
    ...
    if response.status_code == 200:
        ...
        for para in paragraphs:
            if 'country' in para.text.lower() or 'origin' in para.text.lower():
                ...
                place = extract_country_or_place(text)
                if place != "Unknown":
                    return place
    return None


def search_villains_wiki(movie, villain):
...
def search_heroes_wiki(movie, villain):
...
def update_csv_with_villain_origin(csv_path):
    ...
    for index, row in df.iterrows():
        if pd.isna(row['Origin']) or row['Origin'] == 'Unknown':
            ...
            origin_info = search_villains_wiki(movie, villain)
            if not origin_info:
                origin_info = search_heroes_wiki(movie, villain)
            if not origin_info:
...
updated_df = update_csv_with_villain_origin(csv_path)
